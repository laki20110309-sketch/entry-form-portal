...
- [x] ホーム画面の見出しを「大会 エントリー用 フォーム」に変更する
- [x] ホーム画面の説明文を指定文言へ変更する
- [x] 文言変更後の画面を確認して保存する（型チェック・ビルド時に反映確認）
...
- [x] 本番のフォーム検索で発生するDBクエリエラーを原因特定し、既存データを保持して修正する（本番formsスキーマ確認、公開API 200応答確認）
- [x] フォーム検索エラーの回帰テストと本番相当の表示確認を行う（安全確認回帰テスト、型・Vitest、本番公開API確認）
