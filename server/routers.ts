...
import { filterAdminGuilds, getDiscordGuilds, getDiscordSession, getDiscordUser } from "./discordOAuth";
...
export const appRouter = router({
  ...
  publicForms: router({
    ...
    submit: publicProcedure.input(z.object({ slug: z.string(), values: z.record(z.string(), z.union([z.string(), z.array(z.string())])) })).mutation(async ({ input, ctx }) => {
      ...
      const payload = { formTitle: form.title, submissionId, answers: Object.fromEntries(qs.map(q => [q.label, valueMap[String(q.id)] ?? ""])) };
  ...
  admin: router({
    ...
    discordStatus: adminProcedure.query(async ({ ctx }) => { const session = await getDiscordSession(ctx.req); if (!session) return { connected: false as const }; try { const user = await getDiscordUser(session.accessToken); return { connected: true as const, username: user.global_name || user.username }; } catch { return { connected: false as const }; } }),
    discordGuilds: adminProcedure.query(async ({ ctx }) => { const session = await getDiscordSession(ctx.req); if (!session) throw new TRPCError({ code: "PRECONDITION_FAILED", message: "先にDiscordへログインしてください。" }); const [userGuilds, botGuilds] = await Promise.all([getDiscordGuilds(session.accessToken), botFetch("/guilds")]); const botGuildIds = new Set((botGuilds.guilds as Array<{ id: string }>).map(guild => guild.id)); return filterAdminGuilds(userGuilds).filter(guild => botGuildIds.has(guild.id)); }),
    discordChannels: adminProcedure.input(z.object({ guildId: z.string().regex(/^\d{15,25}$/) })).query(async ({ ctx, input }) => { const session = await getDiscordSession(ctx.req); if (!session) throw new TRPCError({ code: "PRECONDITION_FAILED", message: "先にDiscordへログインしてください。" }); const guilds = await getDiscordGuilds(session.accessToken); if (!filterAdminGuilds(guilds).some(guild => guild.id === input.guildId)) throw new TRPCError({ code: "FORBIDDEN", message: "このサーバーを管理する権限がありません。" }); const result = await botFetch(`/guilds/${input.guildId}/channels`); return result.channels as Array<{ id: string; name: string; type: number }>; }),
